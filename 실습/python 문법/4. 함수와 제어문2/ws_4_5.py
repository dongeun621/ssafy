...
def is_validation(data):
    ...
    for black in black_list:
        if black == data['company']:
            return 'block'

    for blood_type in blood_types:
        if blood_type == data['blood_group']:
            blood_group = 1
    if data['mail'].find('@') == -1:
        mail = 0
    ...
    if data['website'] == []:
        website = 0
    if blood_group == 0: result.append('blood_group')
    if mail == 0: result.append('mail')
    if name == 0: result.append('name')
    if website == 0: result.append('website')
